(function (document, window, jQuery, paylixApi) {
  class TablePaginationComponent {
    constructor(selector, pagination, id, tableBlockName, renderOptions) {
      ...
      jQuery(document).on('paylixTablePaginationUpdateEvent', (e, eventInfo) => {
    ...
    init() {
      jQuery(document).trigger('paylixTablePaginationUpdateEvent', { paginationId: this.id });
    }
    ...
    renderProduct(startIndex, endIndex) {
      ...
      paylixApi
        ...
        .then((resp) => {
          ...
          $('.paylix-block.pagination-table').replaceWith($component);
          setTimeout(function () {
            $(document).trigger('paylixTablePaginationUpdateEvent', { paginationId: this.id });
          });
        })
        .catch((resp) => {
  ...
  window.paylixTablePaginationComponent = TablePaginationComponent;
