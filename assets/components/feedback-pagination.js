(function (document, window, jQuery, paylixApi) {
  class FeedbackPaginationComponent {
    constructor(selector, pagination, id, renderOptions) {
      ...
      jQuery(document).on('paylixFeedbackPaginationUpdateEvent', (e, eventInfo) => {
    ...
    init() {
      jQuery(document).trigger('paylixFeedbackPaginationUpdateEvent', { paginationId: this.id });
    }
    ...
    renderProduct(startIndex, endIndex) {
      paylixApi
        ...
        .then((resp) => {
          // $(document).find('.paylix-feedback-wrapper').remove();
          const $component = $(resp.html);
          // $component.insertBefore(this.$pagination);
          $('.paylix-block.feedback_items').replaceWith($component);
          setTimeout(function () {
            $(document).trigger('paylixFeedbackPaginationUpdateEvent', { paginationId: this.id });
          });
        })
        .catch((resp) => {
  ...
  window.paylixFeedbackPaginationComponent = FeedbackPaginationComponent;
