(function (document, window, jQuery, paylixContext, paylixStoreFactory,) {
  class Helper {
    isMobile() {
    ...
    getImageUrl(id, type, isDark) {
      ...
      if (id && `${id}`.toLowerCase() !== 'null') {
        return `https://imagedelivery.net/${paylixContext.get('config', {})['CLOUDFLARE_FOLDER_ID']}/${id}/${type}`;
      }

      const defaultAsset = isDark ? 'loader-logo-light.svg' : 'loaded-logo-dark.svg';
      return paylixContext.get('assets')[defaultAsset];
    }
    ...
    redirectForLocalTest(toSubdomain, pathname) {
      ...
      if (toSubdomain) {
        if (currentSubdomain) {
          subList.shift();
          window.location.href = `http://${toSubdomain}.local-test-paylix.com:3000${pathname}`;
        } else {
          window.location.href = `http://${toSubdomain}.local-test-paylix.com:3000${pathname}`;
        }
      } else {
        if (currentSubdomain) {
          subList.shift();
          window.location.href = `http://local-test-paylix.com:3000${pathname}`;
        } else {
          window.location.href = `http://local-test-paylix.com:3000${pathname}`;
        }
      }
    ...
    redirectToSubLink(toSubdomain, pathname, blank) {
      let host = window.location.host;
      let localTestpaylix = host.includes('local-test-paylix');
      let isStaging = host.includes('paylix.');
  ...
  window.paylixHelper = new Helper();
})(document, window, jQuery, paylixContext, paylixStoreFactory);
